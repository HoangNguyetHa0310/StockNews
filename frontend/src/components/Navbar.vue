<template>
  <nav class="border-b border-theme-border bg-theme-card/90 backdrop-blur-md sticky top-0 z-40 transition-colors duration-200">
    <!-- Widescreen Container: ~80% to 85% width (margin 10-15% hai bên) -->
    <div class="w-full max-w-[92%] xl:max-w-[88%] 2xl:max-w-[82%] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
      
      <!-- Logo & Title -->
      <div class="flex items-center space-x-3">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-500 to-emerald-500 flex items-center justify-center shadow-lg shadow-cyan-500/20 shrink-0">
          <TrendingUp class="w-6 h-6 text-slate-950 stroke-[2.5]" />
        </div>
        <div>
          <div class="flex items-center gap-2">
            <h1 class="text-base sm:text-lg font-bold tracking-tight text-theme-text">
              VN30 QUANT & AI
            </h1>
            <span class="text-[10px] font-mono font-semibold px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30">
              FASTAPI + VUE 3
            </span>
          </div>
          <p class="text-[11px] text-theme-text-muted hidden sm:block">
      ...
      <div class="flex items-center gap-2 sm:gap-2.5">
        ...
        <div 
          ...
          class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold shadow-sm transition"
        ...
        >
          ...
          <span class="md:hidden">{{ marketData.market_schedule.is_trading ? 'Sàn Mở' : 'Đóng Phiên' }}</span>
        </div>

        <!-- Live Auto-Refresh Status Pill (Tự động làm mới ngầm theo chu kỳ config.py) -->
        <div 
          class="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-cyan-600 dark:text-cyan-400 text-xs font-mono font-medium shadow-sm"
        ...
        <button 
          @click="toggleTheme" 
          class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-theme-subtle hover:bg-theme-card-hover text-theme-text border border-theme-border text-xs font-semibold transition shadow-sm"
        ...
        <!-- VN-INDEX Pill -->
        <div v-if="marketData" class="flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-theme-subtle border border-theme-border text-xs shadow-inner">
